...
app.post('/register', async (req, res) => {
    try {
        ...
        await newUser.save();
        // Генерируем токен
        const token = jwt.sign({ userId: newUser._id, login: newUser.login }, SECRET_KEY, { expiresIn: '1h' });
...
const authMiddleware = (req, res, next) => {
    ...
    if (!token)
        return res.status(401).json({ data: { message: 'Токен отсутствует' } });
    try {
...
app.get('/mines/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/combine/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/work_shift/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/month_plan/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// Защищённый маршрут для получения данных о work_shift
app.get('/work_shift/:date/:workShift', authMiddleware, async (req, res) => {
    try {
...
app.get('/samohodniiVagon/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/bunker/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/entry', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ userId: user._id, login: user.login }, SECRET_KEY, // Убедитесь, что используете сложный секретный ключ
